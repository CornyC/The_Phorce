...
class Molecular_system:
    ...
    def __init__(self, mdr_universe, ase_sys, openmm_sys, naked_molecule=None, molecule1=None, molecule2=None):
        ...
        # adding a dict to hold multiple openMM systems
        self.openmm_systems = {
            'system1': openmm_sys,
            'system2': openmm_sys,
            'system3': openmm_sys # can add more systems if requried
        }
    ...
    def generate_qm_energies_forces(self, atomgroup, atomgroup_name, input_paths, cp2k_inp):
        ...
        for frame_nr, frame in enumerate(coords):
            ...
            calc = CP2K(basis_set=None,
                        ...
                        inp=cp2k_inp,
                        print_level='MEDIUM')
            CP2K.command = 'env OMP_NUM_THREADS=6 cp2k_shell.ssmp' # TODO: set OMP_NUM_THREADS somewhere else? (can just call os.environ where it's needed -> currently initialized below the main class)
    ...
    def calculate_qm_net_forces(self, molnames, input_paths, cp2k_input):
        ...
        for molname in molnames.keys(): # aaaahhhhhh need another dict to store shit :(
            self.generate_qm_energies_forces(atom_group, molname, input_paths, cp2k_input)
            ...
            self.generate_qm_charges_energies_forcesraw_optcoords(molname[cp2k_input=input_paths])
            for mdanalysis in input_paths:
                return
    ...
    def generate_mm_energies_forcesraw(self, coords=None):
        ...
        # fill arrays (modifying here to accomodate the openMM system)
        for n in range(len(coords)):
            openmm_sys = self.openmm_systems[system1, system2, system3]
            openmm_sys.run_calculation(coords[n])
